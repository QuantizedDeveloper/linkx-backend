...
def upload_gig(request):
    ...
    try:
        ...
        return Response(
            {"error": "Only freelancers can upload gigs"},
            status=403
        )
    ...
    # 3️⃣ Rate limiting
    if Gig.objects.filter(
        freelancer=profile,
        created_at__gte=now - timedelta(hours=1)
    ).count() >= 2:
        return Response(
            {"error": "Gig upload limit: 2 per hour"},
            status=429
        )
    ...
    if Gig.objects.filter(
        ...
        return Response(
            {"error": "Gig upload limit: 5 per day"},
            status=429
        )
    ...
    return Response(
        {"message": "Gig submitted for approval", "gig_id": gig.id},
        status=201
    )
